---
title: "Testing_Report_Project4_Group3"
...
```{r Load Data and Train-test Split , warning=FALSE, message=FALSE, echo = FALSE}
library(dplyr)
library(tidyr)
library(ggplot2)
library(pracma)
...
<br>
Not applicable to Group 3
<br>
#### Step 2.1 A1 + P3 Algorithm

##### Step 2.1.1 Parameter tuning and visualization for A1

```{r Parameter tuning and visualization for A1 , warning=FALSE, message=FALSE, echo = FALSE}
...
```{r Parameter tuning and visualization for A1 - Continue , warning=FALSE, message=FALSE, echo = FALSE, eval=FALSE}
...
```{r, Parameter tuning and visualization for A1 - ContinueII , warning=FALSE, message=FALSE, echo = FALSE}
...
```{r Parameter tuning result of A1, warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
source("../lib/Matrix_Factorization.r")
...
#### Step 2.1.3 Post-processing by P3-A1

```{r Post-processing by P3-A1 , warning=FALSE, message=FALSE, echo = FALSE}
source("../lib/P3.r")
...
```{r Post-processing by P3-A1-Continue, warning=FALSE, message=FALSE, echo = FALSE}
# SVD_KRR
...
```{r Evaluation , warning=FALSE, message=FALSE, echo = FALSE}
library(ggplot2)
...
```{r Evaluation - Continue , warning=FALSE, message=FALSE, echo = FALSE}
...
```{r Parameter tuning and visualization for A2 , warning=FALSE, message=FALSE, echo = FALSE}
f_list <- c(8, 10)
lp_list <- c(0.1,0.2)
lq_list <- c(0.1,0.2) 
...
```{r Parameter tuning and visualization for A2 - Continue , warning=FALSE, message=FALSE, echo = FALSE}
epochs=100
lrate=0.001
...
```{r Parameter tuning and visualization for A2 - ContinueII , warning=FALSE, message=FALSE, echo = FALSE}
...
```{r Parameter tuning result of A2 , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
...
```{r Post-processing by P3-A2 , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
source("../lib/P3.r")
load("../output/mat_fac_A2.RData")
```

```{r Post-processing by P3-A2 - Continue , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
...
```{r Post-processing by P3-A2 - ContinueII , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
# SVD_KRR
svd_krr_pred_rating <- K(X,X) %*% solve((K(X,X)+lamb*I)) %*% t(rating)
# (mxm)
...
```{r Evaluate A2 by RMSE , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
library(ggplot2)
...
```{r Evaluate A2 P3 , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
source("../lib/Matrix_Factorization.R")
```
```{r}
size(rating)
...
```{r Evaluate A2 P3 - Continue , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
RMSE_mat <- function(T, Y){
 E    = T-Y;
 SQE  = E^2
 MSE  = mean(SQE)
...
```{r Evaluate A2 P3 - ContinueII , warning=FALSE, message=FALSE, echo = FALSE, eval= FALSE}
RMSE_mat(T = rating, Y =t(svd_krr_pred_rating) )
```
